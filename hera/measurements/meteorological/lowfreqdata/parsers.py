...
class Parser_JSONIMS(object):
    ...
    def __init__(self):
        ...
        self._station_column = 'stn_name'
        self._time_coloumn = 'time_obs'
    ...
    def parse(self, fileName, metadatafile=None, **metadata):

        station_column  = self.station_column
        time_column     = self.time_columns

        if os.path.isfile(fileName):
            df = pandas.read_json(fileName)
        else:
            all_files = glob.glob(os.path.join(fileName, "*.json"))
