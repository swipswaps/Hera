...
class preProcess(project.ProjectMultiDBPublic):
    ...
    def extractFile(self,path,time,names,skiphead=20,skipend=4):
        newData = pandas.read_csv(path, skiprows=skiphead,
    ...
    def extractRunResult(self, casePath, cloudName,file=None,save=False,addToDB=True,**kwargs):
        datalist = []
        newData = self.extractFile(f"{casePath}/constant/kinematicCloudPositions",0,['x','y','z'])
    ...
    def getConcentration(self, data,Q=1000000, measureX=10, measureY=10, measureZ = 10, measureTime = 10):

        data = data.copy()
        nParticles = len(data.loc[data.time==data.time.min()])
        ...
        data["time"] = (data["time"] / measureTime).astype(int)*measureTime + measureTime
        data["Dosage"] = Q/nParticles
